package com.safi.datastoresample.utils
...
class PrefDatastore(private val context: Context) {
    ...
    fun observeString(key: String) : Flow<String> {
        return context.dataStore.data.map { preferences ->
            val data = preferences[stringPreferencesKey(key)] ?: ""
            data
        }
    }
    ...
    fun observeInt(key: String) : Flow<Int> {
        return context.dataStore.data.map { preferences ->
            val data = preferences[intPreferencesKey(key)] ?: 0
            data
        }
    }
    ...
    fun observeBoolean(key: String) : Flow<Boolean> {
        return context.dataStore.data.map { preferences ->
            val data = preferences[booleanPreferencesKey(key)] ?: false
            data
        }
    }
